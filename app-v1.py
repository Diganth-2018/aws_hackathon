...
def run_claude_3(query):
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            future1 = executor.submit(retrieve_information, query, kb_id_1,3)
            future2 = executor.submit(retrieve_information, query, kb_id,3)
...
def run_llama_3(query):
    try:
        ...
        bedrock = boto3.client(service_name="bedrock-runtime")
        llm = BedrockChat(model_id="meta.llama3-70b-instruct-v1:0", client=bedrock, model_kwargs={"max_gen_len": 512})
        ...
        def run_compliance(context):
            return result

# Button to display results from Claude 3
if st.button('Run Claude 3'):
    result_claude_3 = run_claude_3(query)
    st.write('Claude 3 Results:')
    st.write(result_claude_3)

# Button to display results from Llama 3
if st.button('Run Llama 3'):
    result_llama_3 = run_llama_3(query)
    st.write('Llama 3 Results:')
    st.write(result_llama_3)
    
# Button to display results from compliance check  
if st.button('Run Compliance'):
    result_compliance = run_compliance(context)
    st.write('Compliance Results:')
    st.write(result_compliance)